...
(function($, window, document, undefined)
{
	...
	Timepicker.prototype =
	{
		...
		_init : function()
		{
			...
			{
				this.$widget = $(this.getTemplate()).appendTo(
					this.$element.parents('.bootstrap-timepicker')).on('click',
		...
		getTemplate : function()
		{
			...
			templateContent = '<table>'
				...
				+ minuteTemplate
				+ '</td> '
				+ (this.showSeconds ? '<td class="separator">:</td>' + '<td>'
					+ secondTemplate + '</td>' : '')
				+ (this.showMeridian ? '<td class="separator">&nbsp;</td>' + '<td>'
					+ meridianTemplate + '</td>' : '')
				+ '</tr>'
				+ '<tr>'
		...
		{
			return this
				.formatTime(this.hour, this.minute, this.second, this.meridian);
		},
		hideWidget : function()
		{
			...
			if (!this.$element.val())
			{
				if (defaultTime === 'current')
				{
					var dTime = new Date(), hours = dTime.getHours(), minutes = Math
						...
						* this.minuteStep, seconds = Math.floor(dTime.getSeconds()
						/ this.secondStep)
		...
		updateWidget : function()
		{
			...
			var hour = this.hour < 10 ? '0' + this.hour : this.hour, minute = this.minute < 10 ? '0'
				+ this.minute
				: this.minute, second = this.second < 10 ? '0' + this.second
				: this.second;
			if (this.showInputs)
			{
			...
			else
			{
				...
				if (this.showMeridian)
				{
					this.$widget.find('span.bootstrap-timepicker-meridian').text(
						this.meridian);
				}
			}
		},
		updateFromWidgetInputs : function()
		{
			...
			else
			{
				var time = $('span.bootstrap-timepicker-hour', this.$widget).text()
					...
					+ (this.showSeconds ? ':'
						+ $('span.bootstrap-timepicker-second', this.$widget).text() : '')
					+ (this.showMeridian ? ' '
						+ $('span.bootstrap-timepicker-meridian', this.$widget).text() : '');
				this.setTime(time);
			}
		},
		...
		widgetKeydown : function(e)
		{
			...
			switch (e.keyCode)
			{
				case 9: // tab
					if (this.showMeridian)
					{
						if (name === 'meridian') { return this.hideWidget(); }
					}
					else
					{
						if (this.showSeconds)
						{
							if (name === 'second') { return this.hideWidget(); }
						}
						else
						{
							if (name === 'minute') { return this.hideWidget(); }
						}
					}
					this.updateFromWidgetInputs();
					break;
	...
	$.fn.timepicker = function(option)
	{
		...
		return this
			.each(function()
			{
				...
				if (!data)
				{
					$this.data('timepicker', (data = new Timepicker(this, $.extend({},
						$.fn.timepicker.defaults, options, $(this).data()))));
				}
